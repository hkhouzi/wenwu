...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (!org) return res.status(404).json({ success: false, message: '组织不存在' })

    // Check access
    const members = await dbAll(
      `SELECT u.id, u.email, u.name, u.role, oi.accepted
       FROM org_invites oi
...
router.put('/:id', [
  ...
  try {
    ...
    if (!org) return res.status(404).json({ success: false, message: '组织不存在' })
    if (org.owner_user_id !== userId) return res.status(403).json({ success: false, message: '只有管理员可以修改组织信息' })
...
router.post('/:id/invite', [
  ...
  try {
    ...
    const org = await dbGet(`SELECT * FROM organizations WHERE id = $1`, [id])
    if (!org) return res.status(404).json({ success: false, message: '组织不存在' })
    if (org.owner_user_id !== userId) return res.status(403).json({ success: false, message: '只有管理员可以邀请成员' })

    // Check if already invited
    const existing = await dbGet(
      `SELECT * FROM org_invites WHERE org_id = $1 AND email = $2 AND accepted = FALSE`,
      [id, normalizedEmail]
    )
    if (existing) return res.status(409).json({ success: false, message: '该邮箱已在邀请列表中' })

    const token = uuidv4()
    await dbRun(
      `INSERT INTO org_invites (id, org_id, email, role, invite_token, invited_by, expires_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`,
      [uuidv4(), id, normalizedEmail, role, token, userId, Date.now() + 7 * 24 * 60 * 60 * 1000]
    )
...
router.post('/invites/:token/accept', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const invite = await dbGet(
      `SELECT * FROM org_invites WHERE invite_token = $1 AND accepted = FALSE`,
      [token]
    )
    if (!invite) return res.status(404).json({ success: false, message: '邀请无效或已过期' })
    if (invite.expires_at && Date.now() > invite.expires_at) return res.status(410).json({ success: false, message: '邀请已过期' })
    if (invite.email !== userEmail) return res.status(403).json({ success: false, message: '此邀请属于其他邮箱账号' })
