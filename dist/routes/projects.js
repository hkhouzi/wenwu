...
const param = (params, val) => { params.push(val); return isPostgres() ? `$${params.length}` : '?'; };
...
router.get('/', async (req, res) => {
    try {
        ...
        if (!orgId)
            return res.status(403).json({ success: false, message: '未加入任何组织' });
        const { name, heritage_type, status, page = 1, size = 20 } = req.query;
        const params = [orgId];
        let where = isPostgres() ? 'WHERE org_id = $1' : 'WHERE org_id = ?';
        if (name) {
            where += ` AND name LIKE ${param(params, `%${name}%`)}`;
        }
        if (heritage_type) {
            where += ` AND heritage_type = ${param(params, heritage_type)}`;
        }
        if (status) {
            where += ` AND status = ${param(params, status)}`;
        }
        const totalRow = await dbGet(`SELECT COUNT(*) as cnt FROM projects ${where}`, params);
        const total = totalRow?.cnt || 0;
        const offset = (Number(page) - 1) * Number(size);
        const items = await dbAll(`SELECT * FROM projects ${where} ORDER BY created_at DESC LIMIT ${param(params, Number(size))} OFFSET ${param(params, offset)}`, params);
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const project = await dbGet(`SELECT * FROM projects WHERE id = ${param(p, id)} AND org_id = ${param(p, orgId)}`, p);
        if (!project)
            return res.status(404).json({ success: false, message: '项目不存在' });
        // Stats
        const stats = await dbGet(`SELECT COUNT(*) as total,
        SUM(CASE WHEN status = 'draft' THEN 1 ELSE 0 END) as draft,
        SUM(CASE WHEN status = 'reviewing' THEN 1 ELSE 0 END) as reviewing,
        SUM(CASE WHEN status = 'approved' THEN 1 ELSE 0 END) as approved
       FROM documents WHERE project_id = ${param([], id)}`, [id]);
...
router.post('/', [
    ...
    try {
        ...
        if (!orgId)
            return res.status(403).json({ success: false, message: '未加入任何组织' });
        const { name, project_code, heritage_type, heritage_level, location, owner_unit, construction_unit, supervision_unit, design_unit, start_date, end_date, status, description } = req.body;
        // Check uniqueness within org
        const exists = await dbGet(`SELECT 1 FROM projects WHERE org_id = ${param([], orgId)} AND project_code = ${param([], project_code)}`, [orgId, project_code]);
        if (exists)
            return res.status(409).json({ success: false, message: '该项目编号已存在' });
        const id = uuidv4();
        ...
        await dbRun(`INSERT INTO projects (${fields}) VALUES (${placeholders})`, vals);
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const { id } = req.params;
        const p = [];
        const project = await dbGet(`SELECT * FROM projects WHERE id = ${param(p, id)} AND org_id = ${param(p, orgId)}`, [id, orgId]);
        if (!project)
            return res.status(404).json({ success: false, message: '项目不存在' });
        const { name, project_code, heritage_type, heritage_level, location, owner_unit, construction_unit, supervision_unit, design_unit, start_date, end_date, status, description } = req.body;
        await dbRun(`UPDATE projects SET
...
router.get('/:id/stats', async (req, res) => {
    try {
       ...
       FROM documents WHERE project_id = ?`, [id]);
