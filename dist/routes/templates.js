...
// ─── Generate doc code ───
async function generateDocCode(projectId, docCategory) {
    ...
    const countRow = await dbGet('SELECT COUNT(*) as cnt FROM documents WHERE project_id = ? AND doc_category = ?', [projectId, docCategory || '']);
...
router.get('/', async (req, res) => {
    try {
        ...
        const params = [];
        if (heritage_type) {
            sql += ' AND (heritage_type = ? OR heritage_type IS NULL)';
...
router.post('/', async (req, res) => {
    try {
        ...
        if (!template_code || !name)
            return res.status(400).json({ success: false, message: '模板编号和名称不能为空' });
        const exists = await dbGet('SELECT 1 FROM templates WHERE template_code = ?', [template_code]);
        if (exists)
            return res.status(409).json({ success: false, message: '模板编号已存在' });
        const id = uuidv4();
        await dbRun(`INSERT INTO templates (id, template_code, name, heritage_type, doc_category, doc_type, table_schema, default_content, description)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [id, template_code, name, heritage_type || null, doc_category || null, doc_type || null, table_schema || null, default_content || null, description || null]);
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (!template)
            return res.status(404).json({ success: false, message: '模板不存在' });
        await dbRun(`UPDATE templates SET name=?, heritage_type=?, doc_category=?, doc_type=?, table_schema=?, default_content=?, description=?, updated_at=CURRENT_TIMESTAMP WHERE id=?`, [name, heritage_type, doc_category, doc_type, table_schema, default_content, description, id]);
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        if (template.original_file_path && fs.existsSync(template.original_file_path))
            fs.unlinkSync(template.original_file_path);
...
router.get('/:id/preview', async (req, res) => {
    try {
        ...
        if (ext !== '.docx')
            return res.status(400).json({ success: false, message: '仅支持 .docx 格式预览' });
...
router.post('/:id/upload', upload.single('file'), async (req, res) => {
    try {
        ...
        if (template.original_file_path && fs.existsSync(template.original_file_path))
            fs.unlinkSync(template.original_file_path);
        await dbRun('UPDATE templates SET original_file_path=?, original_file_name=?, updated_at=CURRENT_TIMESTAMP WHERE id=?', [req.file.path, req.file.originalname, id]);
        const updated = await dbGet('SELECT * FROM templates WHERE id = ?', [id]);
        res.json({ success: true, data: updated });
    }
    catch (error) {
...
// ─── Batch download list ───
router.get('/batch-download/all', async (req, res) => {
    try {
        const templates = await dbAll("SELECT * FROM templates WHERE original_file_path IS NOT NULL AND original_file_path != '' ORDER BY doc_category, template_code");
        if (templates.length === 0)
            return res.status(404).json({ success: false, message: '没有任何已上传原始文件的模板' });
        res.json({
            success: true,
...
router.post('/use', async (req, res) => {
    try {
        ...
        if (!template_id || !project_id)
            return res.status(400).json({ success: false, message: '模板ID和项目ID不能为空' });
        ...
        if (template.table_schema) {
            try {
                schema = typeof template.table_schema === 'string' ? JSON.parse(template.table_schema) : template.table_schema;
            }
            catch {
                schema = null;
            }
        ...
        if (schema?.fields) {
            for (const field of schema.fields) {
                if (field.name === '工程名称' && project.name)
                    fieldValues[field.name] = project.name;
                else if (field.name === '监理单位' && project.supervision_unit)
                    fieldValues[field.name] = project.supervision_unit;
                else if (field.name === '施工单位' && project.construction_unit)
                    fieldValues[field.name] = project.construction_unit;
                else if (field.name === '设计单位' && project.design_unit)
                    fieldValues[field.name] = project.design_unit;
                else if (field.name === '业主单位' && project.owner_unit)
                    fieldValues[field.name] = project.owner_unit;
                else if (field.name === '编号')
                    fieldValues[field.name] = docCode;
            }
        }
        ...
        const id = uuidv4();
        await dbRun(`INSERT INTO documents (id, project_id, template_id, doc_code, doc_name, doc_type, doc_category, content, status)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [id, project_id, template_id, docCode, template.name, template.doc_type, template.doc_category, content, 'draft']);
