    ...
    try {
      ...
      LEFT JOIN templates t ON d.template_id = t.id WHERE 1=1`;
        ...
        if (sort_by === 'custom') {
            ...
            const field = validFields[sort_by] || 'd.doc_category';
...
router.put('/archive/reorder', async (req, res) => {
    try {
        ...
        await Promise.all(orders.map((item) => dbRun("UPDATE documents SET sort_order = ?, updated_at = datetime('now') WHERE id = ?", [item.sort_order, item.id])));
...
router.get('/:id', async (req, res) => {
    try {
        const doc = await dbGet(`SELECT d.*, p.name as project_name, p.project_code, p.construction_unit, p.supervision_unit, p.design_unit, p.owner_unit, p.location as project_location
       FROM documents d LEFT JOIN projects p ON d.project_id = p.id WHERE d.id = ?`, [req.params.id]);
        if (!doc)
            return res.status(404).json({ success: false, message: '资料不存在' });
        if (doc.template_id)
            doc.template_info = await dbGet('SELECT * FROM templates WHERE id = ?', [doc.template_id]);
...
router.post('/', async (req, res) => {
    try {
        ...
        if (!project_id || !doc_name)
            return res.status(400).json({ success: false, message: '项目ID和资料名称不能为空' });
        const id = uuidv4();
                ...
                if (schema?.fields && project) {
                    for (const field of schema.fields) {
                        if (field.name === '工程名称' && project.name)
                            ...
                            fieldValues[field.name] = autoCode;
                    }
                }
                autoContent = JSON.stringify({ fieldValues, signatureDates: {}, schema, defaultContent: template.default_content || '' });
            }
        }
        await dbRun(`INSERT INTO documents (id, project_id, template_id, doc_code, doc_name, doc_type, doc_category, content, status, created_by)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?)`, [id, project_id, template_id || null, autoCode, doc_name, doc_type || null, doc_category || null, autoContent, created_by || null]);
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (!doc)
            return res.status(404).json({ success: false, message: '资料不存在' });
        await dbRun(`UPDATE documents SET doc_code=?, doc_name=?, doc_type=?, doc_category=?, content=?, status=?, updated_at=CURRENT_TIMESTAMP WHERE id=?`, [doc_code, doc_name, doc_type, doc_category, content, status || 'draft', id]);
